...
def _ai_get_openai_config():
    """Read and sanitize OpenAI configuration from Streamlit secrets or environment variables.
    ...
    if api_key is not None:
        ...
        if api_key.lower() in {"", "none", "null", "your_openai_api_key_here", "sk-your-api-key-here"}:
            api_key = None
...
def _ai_mask_key(api_key):
    """Return a safe, non-sensitive API-key display string."""
    if not api_key:
        return "not configured"
    key = str(api_key).strip()
    if len(key) <= 12:
        return "configured, but too short"
    return f"{key[:7]}...{key[-4:]}"


def _ai_openai_key_basic_status(api_key):
    """Lightweight local validation before making an API call."""
    if not api_key:
        return False, "OpenAI API key is not configured."
    if not api_key.startswith(("sk-", "sk-proj-")):
        return False, "OpenAI API key format looks invalid. It should start with sk- or sk-proj-."
    if len(api_key) < 40:
        return False, "OpenAI API key looks too short. Copy the full key from the OpenAI dashboard."
    if "*" in api_key or "..." in api_key:
        return False, "OpenAI API key appears masked or truncated. Paste the full unmasked key."
    return True, "OpenAI API key is configured."


def _ai_format_openai_error(error):
    """Convert OpenAI/API exceptions into clear dashboard-safe messages."""
    msg = str(error)
    low = msg.lower()
    if "401" in msg or "invalid_api_key" in low or "incorrect api key" in low:
        return (
            "OpenAI authentication failed: the configured API key is invalid, revoked, masked, truncated, "
            "or not available in this deployment environment. Create a new key, paste the full key into "
            ".streamlit/secrets.toml or the deployment secrets, then restart the app."
        )
    if "429" in msg or "rate_limit" in low:
        return "OpenAI rate limit or quota reached. Check billing/quota, then retry."
    if "model" in low and ("not found" in low or "does not exist" in low):
        return "The configured OpenAI model is unavailable. Set OPENAI_MODEL to gpt-4o-mini or another enabled model."
    if "connection" in low or "timeout" in low:
        return "OpenAI network connection failed or timed out. Check internet access and deployment egress settings."
    return msg
...
def ai_build_visual_context(*args):
    """Build structured visual-context evidence for map, relationship, trend, and chart-explanation workflows.

    Accepts either ai_build_visual_context(df) or ai_build_visual_context(question, df)
    for backward compatibility with older chatbot calls.
    """
    if len(args) == 1:
        df = args[0]
    elif len(args) >= 2:
...
def ai_try_llm_response(question, df):
    ...
    The LLM receives only focused dashboard context generated from active Streamlit filters.
    It also performs local key validation and returns clear messages for invalid keys.
    """
    ...
    key_ok, key_status = _ai_openai_key_basic_status(api_key)
    if not key_ok:
        return append_eusee_redirect(
            f"⚠️ OpenAI is not ready: {key_status}\n\n"
            "Using the built-in dashboard intelligence only.\n\n"
...
""".strip()
        ...
        # Preferred OpenAI Responses API path, with chat-completions compatibility fallback.
        try:
            ...
            try:
                ...
                # Prefer the first error when it is auth-related; otherwise show the second.
                err_text = str(responses_error)
                if "401" in err_text or "invalid_api_key" in err_text.lower() or "incorrect api key" in err_text.lower():
        ...
        safe_error = _ai_format_openai_error(e)
        return append_eusee_redirect(
            "⚠️ OpenAI ChatGPT connection failed, so I am using the built-in dashboard intelligence only.\n\n"
            f"Connection status: {safe_error}\n\n"
            f"Configured key: {_ai_mask_key(api_key)} | Model: {model}\n\n"
            + local_ai_response(user_question, df)
        )
    ...
    with st.container(key="eusee_ai_right_sidebar"):
        ...
        with st.expander("Advanced tools", expanded=False):
            ...
            with tool_tab4:
                ...
                if st.button("Test OpenAI connection", key="copilot_test_openai_connection", use_container_width=True):
                    api_key, model = _ai_get_openai_config()
                    key_ok, key_status = _ai_openai_key_basic_status(api_key)
                    if not key_ok:
                        st.warning(f"OpenAI test failed before API call: {key_status}")
                    else:
                        try:
                            from openai import OpenAI
                            client = OpenAI(api_key=api_key)
                            test_resp = client.responses.create(
                                model=model,
                                input="Return exactly: EUSEE_OPENAI_OK",
                                max_output_tokens=20,
                            )
                            st.success(f"OpenAI connection OK: {getattr(test_resp, 'output_text', '').strip()}")
                        except Exception as e:
                            st.error(_ai_format_openai_error(e))

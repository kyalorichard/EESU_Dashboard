...
# -------------------------------
def get_cookies_manager():
    if "cookies_manager" in st.session_state:
        return st.session_state["cookies_manager"]

    password = st.secrets.get("cookie", {}).get("password")
    if not password:
        st.error("Cookie password not set in st.secrets.toml.")
        st.stop()

    cookies = EncryptedCookieManager(prefix="myapp", password=password)
...
def auth_ui():
    init_state()
    sidebar = st.sidebar
    refresh_id_token()  # Refresh token on UI load
    ...
    if "user" in cookies:
        ...
        st.session_state.idToken = cookies.get("idToken", None)
                ...
                if not firebase_auth:
                    ...
                    if domain not in PRIVILEGED_DOMAINS:
                        ...
                        try:
                            ...
                            # Save to session & cookies as unverified
                            st.session_state.user = user
                            st.session_state.email = email
                            st.session_state.name = email.split("@")[0].title()

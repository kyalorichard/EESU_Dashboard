...
# ---------------- AI ASSISTANT RIGHT-SIDE COLLAPSIBLE PANEL (STABLE) ----------------
def render_ai_assistant_panel(df):
    """Render a stable right-side collapsible AI assistant.

    This version avoids raw HTML wrappers around Streamlit widgets. Instead, it
    uses a real Streamlit container and styles the container itself using an
    anchor + CSS :has selector. When collapsed, a visible right-side card remains
    on screen, so the assistant does not disappear.
    """
    if "ai_messages" not in st.session_state:
    ...
    st.markdown(f"""
    <style>
    /* Keep the assistant independent from the dashboard grid */
    div[data-testid="stVerticalBlock"]:has(.eusee-ai-right-anchor) {{
        ...
        box-shadow: 0 18px 46px rgba(45, 0, 85, 0.24) !important;
        z-index: 999990 !important;
        padding: 0 12px 14px 12px !important;
    }}

    div[data-testid="stVerticalBlock"]:has(.eusee-ai-collapsed-anchor) {{
        position: fixed !important;
        top: 132px !important;
        right: 18px !important;
        ...
        box-shadow: 0 14px 36px rgba(45, 0, 85, 0.22) !important;
        z-index: 999990 !important;
        padding: 10px !important;
    }}

    .eusee-ai-head-stable {{
        margin: 0 -12px 10px -12px;
        padding: 14px 15px 13px 15px;
        border-radius: 22px 22px 0 0;
        background: linear-gradient(135deg, #2d0055, #660094 56%, #7b2cff);
        color: white;
        font-family: Arial, sans-serif;
    }}
    .eusee-ai-title-stable {{font-size: 17px; font-weight: 900; line-height: 1.2;}}
    .eusee-ai-sub-stable {{font-size: 12px; color: #f1e8ff; margin-top: 4px; line-height: 1.35;}}
    .eusee-ai-pills-stable {{display: flex; flex-wrap: wrap; gap: 6px; margin-top: 10px;}}
    .eusee-ai-pill-stable {{background: rgba(255,255,255,.14); border: 1px solid rgba(255,255,255,.20); padding: 4px 8px; border-radius: 999px; font-size: 11px; font-weight: 800; color: white;}}
    .eusee-ai-status-stable {{display: inline-block; background: {level_color}; color: white; font-size: 11px; font-weight: 900; padding: 5px 9px; border-radius: 999px;}}
    .eusee-ai-card-stable {{background:#ffffff; border:1px solid #eee5ff; border-radius:16px; padding:12px; box-shadow:0 5px 16px rgba(45,0,85,.06); margin-bottom:10px; font-family:Arial,sans-serif;}}
    .eusee-ai-msg-stable {{background:#f7f2ff; border:1px solid #eee5ff; border-radius:16px 16px 16px 5px; padding:10px 12px; font-size:13px; color:#222; margin:8px 28px 10px 0; line-height:1.45; white-space:pre-wrap; font-family:Arial,sans-serif;}}
    .eusee-ai-user-stable {{background:linear-gradient(135deg,#660094,#7b2cff); color:#fff; border-radius:16px 16px 5px 16px; padding:10px 12px; font-size:13px; margin:8px 0 10px 42px; line-height:1.45; white-space:pre-wrap; font-family:Arial,sans-serif;}}
    .eusee-ai-chat-scroll-stable {{max-height: 310px; overflow-y: auto; padding-right: 2px;}}
    .eusee-ai-mini-title {{font-family:Arial,sans-serif; font-weight:900; color:#2d0055; font-size:14px; margin-bottom:4px;}}
    .eusee-ai-mini-note {{font-family:Arial,sans-serif; color:#666; font-size:12px; line-height:1.35; margin-bottom:8px;}}
    .eusee-typing {{display:inline-flex; gap:4px; align-items:center; padding:6px 0;}}
    .eusee-typing span {{width:6px; height:6px; background:#660094; border-radius:50%; display:block; animation:euseeTyping 1.1s infinite ease-in-out;}}
    .eusee-typing span:nth-child(2){{animation-delay:.15s}} .eusee-typing span:nth-child(3){{animation-delay:.3s}}
    @keyframes euseeTyping {{0%,80%,100%{{opacity:.25;transform:translateY(0)}}40%{{opacity:1;transform:translateY(-3px)}}}}

    div[data-testid="stVerticalBlock"]:has(.eusee-ai-right-anchor) div[data-testid="stButton"] button,
    div[data-testid="stVerticalBlock"]:has(.eusee-ai-collapsed-anchor) div[data-testid="stButton"] button {{
        border-radius: 999px !important;
        font-weight: 800 !important;
        min-height: 34px !important;
    }}
    div[data-testid="stVerticalBlock"]:has(.eusee-ai-right-anchor) div[data-testid="stTabs"] button {{
        font-size: 12px !important;
        font-weight: 800 !important;
        padding: 7px 0 !important;
    }}
    div[data-testid="stVerticalBlock"]:has(.eusee-ai-right-anchor) textarea {{font-size:13px !important;}}
    ...
    # Collapsed state: leave a visible right-side card, not a hidden/empty state.
    if not st.session_state.ai_panel_open:
        with st.container():
            st.markdown('<span class="eusee-ai-collapsed-anchor"></span>', unsafe_allow_html=True)
            st.markdown('''
                <div class="eusee-ai-mini-title">💬 EUSEE AI Assistant</div>
                <div class="eusee-ai-mini-note">Collapsed. Open to ask about the current dashboard filters.</div>
            ''', unsafe_allow_html=True)
            if st.button("Open assistant", key="ai_open_right_panel", use_container_width=True):
                st.session_state.ai_panel_open = True
                st.rerun()
        return

    with st.container():
        st.markdown('<span class="eusee-ai-right-anchor"></span>', unsafe_allow_html=True)
        st.markdown(f'''
            <div class="eusee-ai-head-stable">
                <div class="eusee-ai-title-stable">🤖 EUSEE AI Assistant <span style="font-size:10px;background:rgba(255,255,255,.18);border:1px solid rgba(255,255,255,.25);padding:3px 8px;border-radius:999px;margin-left:6px;">Pro</span></div>
                <div class="eusee-ai-sub-stable">Right-side assistant linked to current dashboard filters.</div>
                <div class="eusee-ai-pills-stable">
                    <span class="eusee-ai-status-stable">{level}</span>
                    <span class="eusee-ai-pill-stable">{s['total_alerts']:,} alerts</span>
                    <span class="eusee-ai-pill-stable">{s['countries_count']:,} countries</span>
                    <span class="eusee-ai-pill-stable">{s['negative_pct']}% negative</span>
                </div>
            </div>
        ''', unsafe_allow_html=True)
        ...
        with c1:
            if st.button("Collapse", key="ai_collapse_right_panel", use_container_width=True):
                st.session_state.ai_panel_open = False
                st.rerun()
        with c2:
            if st.button("Clear", key="ai_clear_right_panel", use_container_width=True):
                st.session_state.ai_messages = [{"role": "assistant", "content": "Chat cleared. Ask me about the currently filtered EU SEE dashboard data."}]
                st.session_state.ai_pending_answer = None
                st.session_state.ai_is_typing = False
                st.rerun()
        with c3:
            if st.button("Brief", key="ai_brief_right_panel", use_container_width=True):
                _save_ai_answer("Generate a policy brief.", df)
                st.rerun()

        chat_tab, insights_tab, export_tab, guide_tab = st.tabs(["Chat", "Insights", "Export", "Guide"])

        with chat_tab:
            st.markdown('<div class="eusee-ai-card-stable"><b style="color:#2d0055;">Quick prompts</b></div>', unsafe_allow_html=True)
            quick_questions = {
            ...
            for idx, (label, prompt) in enumerate(quick_questions.items()):
                with qcols[idx % 2]:
                    if st.button(label, key=f"ai_right_quick_{label}", use_container_width=True):
                        _save_ai_answer(prompt, df)
                        st.rerun()

            st.markdown('<div class="eusee-ai-chat-scroll-stable">', unsafe_allow_html=True)
            for msg in st.session_state.ai_messages[-8:]:
                css_class = "eusee-ai-user-stable" if msg["role"] == "user" else "eusee-ai-msg-stable"
                safe_content = _render_chat_content_html(msg["content"])
                st.markdown(f'<div class="{css_class}">{safe_content}</div>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)

            if st.session_state.ai_is_typing and st.session_state.ai_pending_answer:
                st.markdown('<div class="eusee-ai-msg-stable"><div class="eusee-typing"><span></span><span></span><span></span></div><br><b>AI is preparing the answer...</b></div>', unsafe_allow_html=True)
                stream_box = st.empty()
                streamed = ""
                for chunk in _ai_stream_response_text(st.session_state.ai_pending_answer):
                    streamed += chunk
                    stream_box.markdown(f'<div class="eusee-ai-msg-stable">{_render_chat_content_html(streamed)}</div>', unsafe_allow_html=True)
                st.session_state.ai_messages.append({"role": "assistant", "content": st.session_state.ai_pending_answer})
                st.session_state.ai_pending_answer = None
                st.session_state.ai_is_typing = False
                st.rerun()

            with st.form("ai_right_panel_form", clear_on_submit=True):
                user_q = st.text_area("Ask EUSEE AI", placeholder="Ask about countries, regions, trends, actors, mechanisms, or data quality...", height=72, label_visibility="collapsed")
                submitted = st.form_submit_button("Send message", use_container_width=True)
            if submitted and user_q.strip():
                _save_ai_answer(user_q, df)
                st.rerun()
        ...
        with insights_tab:
            st.markdown(f'<div class="eusee-ai-card-stable"><b style="color:#2d0055;">Priority signal</b><br><span class="eusee-ai-status-stable" style="margin-top:8px;">{level}</span><div style="font-size:12px;color:#666;margin-top:8px;">{level_note}</div></div>', unsafe_allow_html=True)
            k1, k2 = st.columns(2)
            with k1:
                render_ai_metric("Total alerts", f"{s['total_alerts']:,}", "Filtered records")
                render_ai_metric("Negative share", f"{s['negative_pct']}%", f"{s['negative']:,} negative alerts", color=level_color)
            with k2:
                render_ai_metric("Countries", f"{s['countries_count']:,}", "Covered in filter")
                render_ai_metric("Regions", f"{s['regions_count']:,}", "Regional coverage")
            with st.expander("AI interpretation", expanded=True):
                st.markdown(_render_chat_content_html(local_ai_response("interpret the current view", df)), unsafe_allow_html=True)
        ...
        with guide_tab:
            st.markdown('''
                <div class="eusee-ai-card-stable">
                    <b style="color:#2d0055;">How to use this assistant</b>
                    <ul style="font-size:12px;color:#333;line-height:1.45;padding-left:18px;">
                        <li>Apply dashboard filters first; answers follow the filtered view.</li>
                        <li>Use Chat for questions and quick prompts.</li>
                        <li>Use Insights for summary signals and quality checks.</li>
                        <li>Use Export for summaries, policy briefs, transcripts and filtered records.</li>
                        <li>Collapse keeps a visible right-side launcher instead of hiding the assistant.</li>
                    </ul>
                </div>
            ''', unsafe_allow_html=True)

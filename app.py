...
def render_cfr_equal_panel_height():
    """
    Synchronize the rendered heights of the CFR upper panels.

    Panels synchronized:
      1. Aggregated CFR scores by principle
      2. Distribution of principles across years
         (same left-hand panel)
      3. Regional score patterns

    Uses the actual rendered Streamlit DOM rather than relying only
    on CSS height declarations.
    """
    components.html(
        r"""
        <script>
        (function () {
            ...
            const LEFT_MARKER = ".cfr-chart-panel-marker";
            const RIGHT_MARKER = ".cfr-region-panel-marker";

            const STYLE_ID = "eusee-cfr-equal-panel-height-style";
            ...
            function equalizePanels() {
                ...
                const leftPanel = getPanel(LEFT_MARKER);
                const rightPanel = getPanel(RIGHT_MARKER);
                ...
                /*
                 * First remove the previously applied height so that
                 * we can measure the natural rendered height.
                 */
                leftPanel.classList.remove(
                    "eusee-cfr-equalized-panel"
                );
                ...
                /*
                 * Temporarily allow both panels to determine their
                 * natural height.
                 */
                ...
                /*
                 * Use the larger of the two actual rendered heights.
                 */
                const equalHeight =
                    Math.ceil(
                        Math.max(
                ...
                if (!equalHeight || equalHeight < 100) {
                    return false;
                }


                /*
                 * Apply the exact same height to both panels.
                 */
                [
                    leftPanel,
                    rightPanel
            ...
            /*
             * Streamlit can render/update the chart and table after
             * the initial DOM load, so try repeatedly for a short
             * period.
             */
            let attempts = 0;
            ...
            /*
             * Re-equalize when Streamlit changes the DOM.
             */
            const observer =
                new MutationObserver(function () {

                    window.clearTimeout(
                        window.__euseeCfrHeightTimer
                    );
...
def render_cfr_analysis():
    ...
    # Synchronize the actual rendered panel heights.
    render_cfr_equal_panel_height()

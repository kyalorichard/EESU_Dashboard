...
if "Select All" in selected_years:
    available_months = sorted(data['month_name'].dropna().unique(),
                              key=lambda m: pd.to_datetime(m, format='%B').month)
else:
    available_months = sorted(data[data['year'].isin(selected_years)]['month_name'].dropna().unique(),
                              key=lambda m: pd.to_datetime(m, format='%B').month)
...
def render_summary_cards(df, base_bar_height=25):
    ...
    """
    with col1:
        st.markdown(f"<div style='{card_style}'><p style='margin:0;font-size:20px;'>Monitored Countries</p><h2 style='margin:6px 0;'>{total_countries}</h2></div>", unsafe_allow_html=True)
    with col2:
        st.markdown(f"<div style='{card_style}'><p style='margin:0;font-size:20px;'>Total Alerts</p><h2 style='margin:6px 0;'>{total_alerts}</h2></div>", unsafe_allow_html=True)
    with col3:
        st.markdown(f"""
...
# ---------------- CHART FUNCTIONS ----------------
# (Keep your create_bar_chart and create_h_stacked_bar functions as in original code)
...
with tab1:
    ...
    a3 = filtered_global.groupby(["region","alert-impact"]).size().reset_index(name='count')  # region now
...
with tab2:
    ...
    r1c1.plotly_chart(create_bar_chart(t1,"Actor of repression","count",horizontal=False),use_container_width=True, key="tab2_chart1")
    r1c2.plotly_chart(create_bar_chart(t2,"Subject of repression","count",horizontal=False),use_container_width=True, key="tab2_chart2")
    r1c3.plotly_chart(create_bar_chart(t3,"Mechanism of repression","count",horizontal=False),use_container_width=True, key="tab2_chart3")
...
with tab5:
    ...
    if geo_file.exists():
        ...
        df_map = filtered_global.groupby(["alert-country","iso_alpha3","region"]).size().reset_index(name="count")
        ...
        fig_map = px.choropleth(
            df_map, geojson=countries_gj, locations='iso_alpha3', color='region',
            hover_name='alert-country', hover_data=['count'],
            color_discrete_sequence=px.colors.qualitative.Pastel,
        )
        fig_map.update_geos(fitbounds="locations", visible=False, center=center)
        fig_map.update_layout(margin={"r":0,"t":0,"l":0,"b":0})
        st.plotly_chart(fig_map, use_container_width=True)
    else:
        st.warning("GeoJSON file not found for the map.")

...
def render_sankey(df, top_n=None, width=900):
    """
    Renders a Sankey diagram for Negative Events with visible and readable labels.
    ...
    # ---------------- PREPARE DATA ----------------
    df_exploded = explode_multi_valued_columns(df, ["Actor of repression", "Mechanism of repression", "Subject of repression"])
    ...
    node_font_colors = [get_contrast_color(c) for c in node_colors]
    ...
    for _, row in df_am.iterrows():
        links.append(dict(
            source=node_index[truncate_label(f"Actor: {row['Actor of repression']}")],
            target=node_index[truncate_label(f"Mechanism: {row['Mechanism of repression']}")],
            value=row["value"]
        ))
    # Mechanism -> Subject
    ...
    for _, row in df_ms.iterrows():
        links.append(dict(
            source=node_index[truncate_label(f"Mechanism: {row['Mechanism of repression']}")],
            target=node_index[truncate_label(f"Subject: {row['Subject of repression']}")],
            value=row["value"]
        ))

    # ---------------- CREATE FIGURE ----------------
    fig_height = max(600, len(nodes)*50)  # dynamically adjust height
    ...
    fig = go.Figure(go.Sankey(
        ...
        node=dict(
            ...
            label=nodes,
            color=node_colors,
            font=dict(color=node_font_colors, size=12),

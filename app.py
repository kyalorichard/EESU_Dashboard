...
def render_summary_cards(df):
    ...
    for i, card in enumerate(cards):
        ...
        if card.get("negative") is not None:
            ...
            <p style="font-size:{font_size_label}px; margin:0;">{card['label']}</p>
            <div style="display:flex; justify-content:space-between; margin:5px 0; font-size:{font_size_value}px;">
                <span class="alert-dot" style="color:#FF4C4C;">● {card['negative']}</span>
                <span class="alert-dot" style="color:#00FFAA;">● {card['positive']}</span>
            </div>
            <div class="summary-bar-container">
                <div class="summary-bar-negative" style="width:{neg_pct}%">{neg_pct}% Negative</div>
                <div class="summary-bar-positive" style="width:{pos_pct}%">{pos_pct}% Positive</div>
            </div>
            """, unsafe_allow_html=True)
...
/* Optional: consistent column spacing */
.css-1d391kg {
    gap: 1rem;
}
...
.summary-bar-container {
    ...
    font-size: 12px;
    font-weight: bold;
    background: #ddd;           /* base bar color */
}

/* Negative and Positive bar segments */
.summary-bar-negative {
    background: #FF4C4C;
    color: white;
    display: flex;
    justify-content: center;
...
with tab1:
    ...
    r2c1.plotly_chart(create_h_stacked_bar(a3,y="continent",x="count",color_col="alert-impact", horizontal=False),use_container_width=True,  key="tab1_chart3")

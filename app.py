...
if 'selected_alert_types_multi' not in st.session_state:
    st.session_state.selected_alert_types_multi = list(data['alert-type'].dropna().unique())
...
# Robust multiselect for alert-types to avoid StreamlitAPIException
alert_impact_options = data['alert-type'].dropna().unique()
valid_default_multi = [val for val in st.session_state.selected_alert_types_multi if val in alert_impact_options]
selected_alert_types_multi = st.sidebar.multiselect(
    "Select Alert Types (Multi)",
    options=alert_impact_options,
    default=valid_default_multi if valid_default_multi else list(alert_impact_options)
)

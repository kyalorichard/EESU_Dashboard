...
with tab5:
    ...
    if geo_file.exists():
        ...
        # Aggregate map data
        df_map = filtered_global.groupby(["alert-country", "iso_alpha3", "region"]).agg(
        ...
        df_map["total_alerts"] = pd.to_numeric(df_map["total_alerts"], errors='coerce').fillna(0)
        df_map["negative_alerts"] = pd.to_numeric(df_map["negative_alerts"], errors='coerce').fillna(0)
        df_map["positive_alerts"] = pd.to_numeric(df_map["positive_alerts"], errors='coerce').fillna(0)
        ...
        if not df_map.empty:
            ...
            fig.update_layout(margin={"r":0,"t":0,"l":0,"b":0}, height=max(400, len(df_map)*25))
            ...
            fig.update_traces(
                hovertemplate=(
                    ...
                    "Positive: %{customdata[2]}<br>"
                    "% Negative: %{customdata[3]}<extra></extra>"
                ),
                customdata=df_map[["total_alerts","negative_alerts","positive_alerts","perc_negative","region"]].values
            )
        ...
        st.warning("GeoJSON file not found for the map visualization.")
